...
Internet traffic reaches `myh.guru` through Cloudflare Tunnel and Gunicorn on `127.0.0.1:5000`. The panel controls per-site Docker Compose projects whose HTTP ports bind only to loopback. Each project has a private bridge; database-capable runtimes also join the external `hosting-databases` network (`172.23.0.0/16`, bridge `myh-db0`, ICC disabled).
