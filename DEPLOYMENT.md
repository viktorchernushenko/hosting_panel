...
The canonical service is `myh-guru.service`; public ingress is `cloudflared.service`. Apply Python/template changes with `sudo systemctl restart myh-guru`, then verify local `/`, `/login`, authenticated routes, service logs, and the public hostname. Runtime stacks are generated by `runtime_engine.py`; do not hand-edit generated Compose files.
